#!/usr/bin/env bash
# run_generation_headless.sh — the US-GE2 generation dock LOGIC-LAYER gate.
#
# Runs generation_test.gd end-to-end through a real Godot binary when one is on PATH.
...
# addons/insimul/editor/generation -> addons/insimul/editor -> addons/insimul -> addons -> insimul (pkg root)
...
if [[ -z "$GODOT" ]]; then
	echo "generation: no godot binary on PATH — SKIP (structural lint + npm-test view-models cover the .gd on a bare box)"
	exit 0
fi
...
[application]
config/name="insimul-generation-test"
PROJ

echo "generation: running headless test with $GODOT ..."
...
"$GODOT" --headless --path "$proj" \
	-s "addons/insimul/editor/generation/generation_test.gd"
