#!/usr/bin/env bash
# run_activation_tests.sh — the genre-bundle ACTIVATION gate (tasklist 147, US-3).
#
# Builds libinsimulcore (QuickJS + the vendored @insimul/core bundle) together
# with test/test_module_activation.cpp under a plain C++ toolchain — no cmake,
# no scons, no godot-cpp, no Godot binary — and executes:
#
#   conformance/modules/genre-activation.json   core's committed genre -> active
#                                module set, resolved here through
#                                `modules.activate` for every bundle and
#                                deep-compared. A vendored table nothing runs is
#                                a checked-in file (this repo has shipped that).
#   the KB witness               for every genre x pack: the pack's signature
#                                predicate is defined in a REAL Trealla KB
#                                exactly when its module is active. "An inactive
#                                module contributes nothing" is only provable
#                                where the nothing would be.
#   templates/project/insimul/scenarios/*.json
#                                the playable scene's own steps, replayed
#                                through the same rows the scene calls.
#
# Like run_corpus_tests.sh it LINKS libinsimul and FAILS LOUDLY when the library
# is absent rather than skipping — a gate that cannot fail is worse than no gate.
#
...
#
# Usage:
#   bash gdextension/test/run_activation_tests.sh [<repo root>]
set -euo pipefail
...
echo "Compiling libinsimulcore + the activation gate ..."
...
"$CXX" -std=c++17 -Wall -Wextra -O1 -g \
	...
	"$root/test/test_module_activation.cpp" \
	"$out"/*.o \
	"$insimul_lib" \
	-o "$out/test_module_activation"

# The gate reads two trees — conformance/ and templates/ — so it takes the repo
# root rather than a corpus directory. An explicit path overrides it.
"$out/test_module_activation" "${1:-$repo}"
